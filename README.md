...
专业验收不会因为用户是新手而降低。Skill 仍会检查读秒、动作完整性、人物和道具一致、空间方向、动作开始／完成状态、参考图职责、声音与剪辑，只是不把复杂规则一次性压给用户。
...
制作文档使用统一外壳；可拍摄剧本和完整分镜脚本例外，统一输出UTF-8 CSV。短内容直接给`csv`代码块，长内容主动创建`.csv`文件，避免部分模型无法生成或读取复杂文档：

| CSV类型 | 必须包含 |
|---|---|
| 可拍摄剧本CSV | 场次、场景、人物、剧情目的、节拍、可见动作、说话人、完整对白、声音和连续性 |
| 完整分镜CSV | 分镜段落、10秒／15秒规格、段内镜头、时间码、景别机位运镜、人物道具、动作状态、对白声音、参考职责、Seedance 2.0提示词和自检 |
...
- 输出带资产、动作开始状态、动作完成状态、机位、对白和声音的完整分镜脚本。

### 6. 视觉与资产设计

- 全局画风锁定：真人电影、3D国风、3A游戏CG、二维动画及自定义风格。
- 核心角色三视图、面部特写和身份细节；所有人物提示词必须包含人物编号和名字。
- GPT可生成顶部显示“项目名／人物名字／资产编号”，并包含多视图、剧情状态、服装细节和表情特写的综合人物资产卡。
- Gemini及其他工具默认使用更稳定的简化三视图或角色卡，不强制复杂文字整板。
...
- 进入故事板阶段前先询问或读取用户的生图工具。
- GPT：可生成中央3×3九宫格、左侧运镜路线、右侧人物／色板／空间参考、底部技术卡的专业电影故事板工作板。
- Gemini：只建议生成干净的简化3×3连续关键帧。
- 其他工具：默认不建议生成多格故事板，改为站位俯视图＋正面站位图。
- 明确“九宫格不等于九次切镜”，支持一镜到底式连续关键状态。
- 锁定人物左右、持物手、场景锚点、运动方向和动作状态。
- 为版式、角色、场景、道具、站位和动作参考分配唯一职责。

## V2.4新增

- 新增生图工具选择门：GPT、Gemini和其他工具走不同的故事板路线。
- 新增GPT专业九宫格电影故事板工作板模板。
- 新增非GPT工具的站位俯视图＋正面站位图模板。
- 新增GPT综合人物资产卡模板，强制显示人物名字和资产编号。
- 所有人物提示词、文件名和资产小结都必须包含人物名字与资产编号。

## V2.5新增

- 视频生成阶段锁定为Seedance 2.0单一输出格式，不再提供I2V、FLF2V、WAN、VACE或其他模型版本。
- 实际可复制提示词只使用Seedance 2.0自然语言和真实上传素材的`@图片`、`@视频`、`@音频`引用。
- 生产提示词和下一步选项中禁止出现“首帧引用”“尾帧引用”、`CUT`、`TAIL`、`END FRAME`等旧字段。
- 新增可直接复用的[Seedance 2.0提示词模板](assets/seedance2-prompt-template.md)。
- 剧本和完整分镜脚本锁定为CSV输出，并提供固定表头模板。
...
- 内部导演计划和外部可复制提示词分层。
- 视频生成提示词只输出Seedance 2.0格式，不再生成任何其他视频模型版本。
- 使用`@图片1`、`@视频1`、`@音频1`绑定真实上传素材，每个引用只承担一个明确职责。
- 按10秒／15秒规格写时间段、动作、运镜、对白、声音、画面和生成要求。
- 最终只交付一个干净的自然语言代码块，不含首尾帧字段、`CUT`、`TAIL`、XML、资产分析或自检日志。
...
- 修改后同步更新受影响的分镜、故事板、动作状态和Seedance 2.0提示词。
...
| 分镜脚本 | 故事板、生图提示词、站位图和动作状态说明 |
| 故事板和参考图 | 唯一一份Seedance 2.0自然语言提示词 |
...
它们分别检查时间码与对白容量、项目阶段证据、提示词输入包缺项与引用冲突、相邻制片段继承状态。脚本不会取代导演对剧情、表演和美感的判断。
...
│   ├── screenplay-csv-template.csv
│   ├── shot-script-csv-template.csv
│   ├── seedance2-prompt-template.md
...
是。当前视频生成阶段锁定为Seedance 2.0单一格式，以避免新手在不同平台语法之间混用。图片与故事板仍会根据GPT、Gemini或其他生图工具选择合适方案。
