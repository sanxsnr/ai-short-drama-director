...
仓库内提供六个面向制作流程的无第三方依赖 Python 验证器：
...
它们分别检查时间码与对白容量、跨SHOT／SEG状态继承、人物—对象—移动—摄影机空间几何、30度适用性与视觉差异路径、项目阶段证据，以及提示词输入包缺项与引用冲突。`scripts/validate_rule_sources.py`作为仓库内部CI检查，防止规则重复或跨文件矛盾。脚本不会取代导演对剧情、表演和美感的判断。
