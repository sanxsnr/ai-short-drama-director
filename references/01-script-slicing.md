...
- 人物坐标、站位、朝向、轴线、状态继承、场景世界机位或信息可见性；这些只由`04-blocking-continuity.md`定义。
...
涉及完整分镜、段内SHOT、故事板格序或机位变化时，必须按`04 → 14 → 13`读取。本文件不得改写或覆盖它们。
...
2. 04：锁定世界空间、轴线、人物／道具状态与信息可见性
3. 14：验证每个SHOT的导演任务、动作覆盖、场景锚点与状态机
4. 13：决定SHOT边界、CUT类型和相邻SHOT视觉差异路径
5. 本文件：把已通过审核的SHOT封装成SEG并读秒
6. 下游模块：生成故事板与视频提示词
```
