...
- `visible_anchor_ids`：画面真实可见的固定锚点集合。
- 朝向、高度和轴线侧。
- `C`与人物正面`F`的关系。
...
自然语言`scene_label`、`zone_label`只用于显示。`刑房`与`刑房内`、`门口`与`门槛内`等文字差异不能自动代表场景或区域发生变化。
...
- 景别从全景自然变为中景，或从极近特写连续展开到全身，只是该摄影机路径的结果，不在本文件自动推导CUT。
- 摄影机到达目标构图后若要求固定，必须记录`lock_after_move=true`，后续不得漂移为新机位。
...
空间连续性不得只比较自然语言。每个持续场景至少建立：

```text
scene_id：唯一场景ID
zone_id：功能区ID
anchor_id：门、窗、床、柱、桌、木桩、门槛等固定锚点ID
time_id：连续时间段ID
```
...
状态转换必须连续。禁止`inside → 再次ENTER`、`raised → 再次抬头`、`held → 再次从原处拿取`。具体动作是否被当前构图完整展示，由`14`验证。
...
关键信息不可见时必须重设空间机位；独立细节镜头先由`14`验证任务证据，再由`13`决定是否通过CUT切入。
...
- visible_anchor_ids：
- 场景观察签名：
- 人物局部几何：C与F关系、推导可见面、屏幕关系、可见／不可见信息。
